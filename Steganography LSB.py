...
class Stegno:
    ...
    def frame2_decode(self, d_f2):
        ...
        if not myfile:
            ...
            l2 = Label(d_f3, text='Enter Password:')
            l2.config(font=('courier', 18))
            l2.grid(pady=5)
            password_entry = Entry(d_f3, show="*")
            password_entry.grid(pady=5)

            decode_button = Button(d_f3, text='Decode', command=lambda: self.show_hidden_data(d_f3, myimg, password_entry.get()))
            decode_button.config(font=('courier', 18))
            decode_button.grid(pady=10)

            back_button = Button(d_f3, text='Cancel', command=lambda: Stegno.home(self, d_f3))
            back_button.config(font=('courier', 18))
            back_button.grid(pady=15)
    ...
    def show_hidden_data(self, d_f3, myimg, password):
        hidden_data = self.decode(myimg)
        try:
            data, file_password = hidden_data.rsplit("|", 1)
            if password != file_password:
                messagebox.showerror("Error", "Incorrect Password!")
                return
        except ValueError:
            messagebox.showerror("Error", "No password found in the image!")
            return
    ...
    def frame2_encode(self, f2):
        ...
        if not myfile:
            ...
            text_area.grid(pady=5)

            l2 = Label(ep, text='Enter Password:')
            l2.config(font=('courier', 18))
            l2.grid(pady=5)
            password_entry = Entry(ep, show="*")
            password_entry.grid(pady=5)

            encode_button = Button(ep, text='Encode', command=lambda: [self.enc_fun(text_area, myimg, password_entry.get()), Stegno.home(self, ep)])
            encode_button.config(font=('courier', 11))
            encode_button.grid(pady=15)

            back_button = Button(ep, text='Cancel', command=lambda: Stegno.home(self, ep))
